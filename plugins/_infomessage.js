...
const foto = `🖼️ Se ha cambiado la *imagen del grupo*.\n👤 Acción hecha por:\n> » @${usuario.split('@')[0]}`
const edit = `⚙️ @${usuario.split('@')[0]} ha cambiado la configuración del grupo.\n🔐 Ahora pueden configurar: *${m.messageStubParameters[0] == 'on' ? 'solo admins' : 'todos'}*`
const newlink = `🔗 El *enlace del grupo* ha sido restablecido.\n👤 Acción hecha por:\n> » @${usuario.split('@')[0]}`
const status = `🚪 El grupo ha sido *${m.messageStubParameters[0] == 'on' ? 'cerrado 🔒' : 'abierto 🔓'}* por @${usuario.split('@')[0]}.\n💬 Ahora pueden enviar mensajes: *${m.messageStubParameters[0] == 'on' ? 'solo admins' : 'todos'}*`
const admingp = `👑 @${users.split('@')[0]} ahora es *admin del grupo*.\n🧩 Acción hecha por:\n> » @${usuario.split('@')[0]}`
const noadmingp = `⚠️ @${users.split('@')[0]} ha dejado de ser *admin del grupo*.\n🧩 Acción hecha por:\n> » @${usuario.split('@')[0]}`
